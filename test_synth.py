...
class TestSynth(unittest.TestCase):
    ...
    def test_emphasis_ext_enabled(self):
        """Does the code appear to have Ext "Emphasis" implemented and enabled by default?"""

        args = default_args()
        args.phrase = 'hello {there}'
        try:
            wav_emph = synthesise_to_wav(args)
        except:
            self.fail('It appears Ext "Emphasis" is not implemented/enabled')

        args.phrase = 'hello there'
        wav_noemph = synthesise_to_wav(args)

        self.assertNotEqual(max(wav_noemph.data), max(wav_emph.data),
                            'It appears Ext "Emphasis" is not implemented/enabled')
